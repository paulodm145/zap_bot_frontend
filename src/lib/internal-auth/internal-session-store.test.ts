...
describe('internalSessionStore', () => {
  ...
  it('keeps the second-factor state separate from an authenticated session', () => {
    ...
    expect(internalSessionStore.getSnapshot()).toEqual({ accessToken: null, stateToken: 'state-token', requiresConfiguration: true, status: 'second_factor' });

    internalSessionStore.authenticate('internal-token');
    expect(internalSessionStore.getSnapshot()).toEqual({ accessToken: 'internal-token', stateToken: null, requiresConfiguration: false, status: 'authenticated' });
  });
  ...
  it('notifies subscribers and returns to the anonymous state', () => {
    ...
    expect(internalSessionStore.getSnapshot()).toEqual({ accessToken: null, stateToken: null, requiresConfiguration: false, status: 'anonymous' });
    unsubscribe();
  });
});
