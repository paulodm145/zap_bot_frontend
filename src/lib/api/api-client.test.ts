...
describe('apiRequest during impersonation', () => {
  ...
  it('does not send cookies or refresh an expired temporary session', async () => {
    sessionStore.impersonate(
      'temporary-token',
      { id: 'user-1', nome: 'Admin', email: 'admin@test.dev', tenantId: 'tenant-1' },
      { tenantName: 'Empresa', sessionId: 'session-1', expiresInSeconds: 900 },
    );
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify({ erro: { codigo: 'NAO_AUTORIZADO', mensagem: 'Expirada' } }), {
          status: 401,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://localhost:3000/api/v1/fluxos',
...
describe('apiRequest authentication security', () => {
  ...
  it('does not send refresh cookies on ordinary API requests', async () => {
    sessionStore.authenticate('access-token', {
      id: 'user-1',
      nome: 'User',
      email: 'user@test.dev',
      tenantId: 'tenant-1',
  ...
  it('shares one refresh request and retries each request only once', async () => {
    sessionStore.authenticate('expired-token', {
      id: 'user-1',
      nome: 'User',
      email: 'user@test.dev',
      tenantId: 'tenant-1',
  ...
  it('exposes retry delay from a rate-limited response', async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ erro: { codigo: 'LIMITE_TENTATIVAS', mensagem: 'Aguarde' } }), {
          status: 429,
