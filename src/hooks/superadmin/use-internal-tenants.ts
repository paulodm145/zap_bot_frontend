...
export const internalTenantKeys = { all: ['superadmin', 'tenants'] as const, list: (params: InternalTenantListParams) => ['superadmin', 'tenants', 'list', params] as const };

export function useInternalTenants(params: InternalTenantListParams) {
  return useQuery({
    queryKey: internalTenantKeys.list(params),
    queryFn: ({ signal }) => {
      const query = new URLSearchParams({ skip: String(params.skip), take: String(params.take), ordenarPor: params.orderBy ?? 'updated_at', ordem: params.order ?? 'desc' });
