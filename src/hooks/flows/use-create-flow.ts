...
export function useCreateFlow() {
  ...
  return useMutation({
    mutationFn: (name: string) => apiRequest<FlowDetail>('/fluxos', {
      method: 'POST',
      body: JSON.stringify({
        nome: name,
        definicao: { schemaVersao: 1, noInicial: 'inicio', nos: [{ id: 'inicio', tipo: 'mensagem', dados: { texto: 'Olá! Como posso ajudar?' } }] },
      }),
    }),
