...
export function useSectors(search='',skip=0,take=20){return useQuery({queryKey:[...sectorKeys.all,search,skip,take],queryFn:({signal})=>apiRequest<Page<Sector>>(`/setores?skip=${skip}&take=${take}&busca=${encodeURIComponent(search)}`,{signal})});}
