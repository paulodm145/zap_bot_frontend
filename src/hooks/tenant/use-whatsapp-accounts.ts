...
export type WhatsAppAccount = { public_id:string; nome:string; phone_number_id:string; waba_id:string; numero_exibicao?:string|null; fluxo_entrada?:{public_id:string;nome:string}|null; status:'PENDENTE'|'VALIDADA'|'INVALIDA'; ativo:boolean; ultima_validacao_at?:string|null; ultimo_erro_mensagem?:string|null };
...
export function useCreateWhatsApp(){ return useAccountAction((input:{nome:string;phoneNumberId:string;wabaId:string;numeroExibicao:string;versaoGraphApi:string;accessToken:string;fluxoEntradaPublicId?:string})=>apiRequest('/contas-whatsapp',{method:'POST',body:JSON.stringify(input)})); }
