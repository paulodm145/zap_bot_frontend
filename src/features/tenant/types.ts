...
export type Conversation = { public_id:string; status:'BOT'|'AGUARDANDO_ATENDENTE'|'COM_ATENDENTE'|'ENCERRADA'; contato:{nome?:string|null;telefone:string}; setor?:{public_id:string;nome:string}|null; atendente?:{public_id:string;nome:string}|null; ultima_mensagem?:string|null; updated_at:string };
export type Message = { public_id:string; direcao:'ENTRADA'|'SAIDA'; tipo:string; texto?:string|null; conteudo?:string|null; status?:string; created_at:string };
