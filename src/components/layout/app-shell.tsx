      ...
      <div className={styles.shell}>
        ...
        <aside className={`${styles.sidebar} ${open ? styles.open : ''}`}>
          ...
          <div className={styles.workspace}>
            <span>{session.user?.nome.slice(0, 2).toUpperCase() ?? 'EA'}</span>
            <div>
              <small>Workspace</small>
              <strong>{session.impersonation?.tenantName ?? 'Meu tenant'}</strong>
            </div>
            <ChevronDown size={15} />
          </div>
          <nav aria-label="Navegação principal">
            {nav
              .filter((item) => !item.roles || (me.data && item.roles.includes(me.data.papel)))
              .map((item) => {
          ...
          <div className={styles.sidebarBottom}>
            ...
            <button className={styles.logout} onClick={() => logout.mutate()} disabled={logout.isPending}>
              <LogOut size={19} />
              {logout.isPending ? 'Saindo...' : 'Sair'}
            </button>
            <div className={styles.profile}>
              <span>{session.user?.nome.slice(0, 2).toUpperCase() ?? 'US'}</span>
              <div>
                <strong>{session.user?.nome ?? 'Usuário'}</strong>
                <small>Conta do tenant</small>
              </div>
              <ChevronDown size={15} />
            </div>
          </div>
        ...
        <div className={styles.content}>
          {session.impersonation && (
            <div className={styles.impersonation}>
              <ShieldCheck size={16} />
              <strong>Acessando como {session.impersonation.tenantName}</strong>
              <span>Sessão administrativa temporária</span>
              <button
                onClick={() => {
                  sessionStore.clear();
                  queryClient.clear();
          ...
          <header className={styles.topbar}>
            ...
            <div className={styles.actions}>
              {actions}
              <Button variant="ghost" size="icon" aria-label="Notificações" icon={<Bell size={19} />} />
