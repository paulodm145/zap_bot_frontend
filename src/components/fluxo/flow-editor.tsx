...
import { useFlowBlockCatalog, type FlowBlockCatalogItem, type FlowBlockType } from '@/hooks/flows/use-flow-block-catalog';

type FlowData = FlowNodeData;
type Tool = { type: FlowBlockType; label: string; detail: string; icon: typeof MessageSquareText; tone: string; content: string };
...
function FlowNode({ id, data, selected }: NodeProps<Node<FlowData>>) {
  ...
  return <div className={`${styles.node} ${selected ? styles.selected : ''} ${data.validationError ? styles.invalidNode : ''}`} title={data.validationError}><NodeToolbar isVisible={selected} position={Position.Right}><button type="button" className={styles.removeNode} onClick={(event) => { event.stopPropagation(); void deleteElements({ nodes: [{ id }] }); }} aria-label={`Remover bloco ${data.detail}`}><Trash2 size={14} />Remover</button></NodeToolbar><Handle type="target" position={Position.Top} /><div className={`${styles.nodeIcon} ${styles[data.kind]}`}><Icon size={17} /></div><div><small>{data.label}</small><strong>{data.detail}</strong>{data.validationError && <em>{data.validationError}</em>}</div>{data.kind !== 'team' && <Handle type="source" position={Position.Bottom} />}</div>;
}
...
const prototypeNodes: Node<FlowData>[] = [
  ...
  { id: 'no_4', type: 'flowNode', position: { x: 530, y: 345 }, data: { label: 'Direcionar setor', detail: 'Escolha um setor', kind: 'team', icon: 'team', content: '', sectorId: '' } },
];
...
function catalogItemToTool(item: FlowBlockCatalogItem): Tool {
  ...
  return { type: item.tipo, label: item.nome, detail: item.descricao, ...visual, content: String(data.texto ?? data.mensagem ?? '') };
}
...
function FlowEditorContent({ flowId, flowName, initialNodes, initialEdges }: { flowId?: string; flowName: string; initialNodes: Node<FlowData>[]; initialEdges: Edge[] }) {
  ...
  const save = useCallback(async () => {
    ...
    if (missingSector) {
      setNodes((current) => current.map((node) => node.id === missingSector.id ? { ...node, data: { ...node.data, validationError: 'Selecione o setor que receberá a conversa.' } } : node));
  ...
  function createNode(tool: Tool, position: { x: number; y: number }) {
    ...
    const node: Node<FlowData> = { id, type: 'flowNode', position, data: { label: tool.label, detail: `Novo bloco de ${tool.label.toLowerCase()}`, kind: tool.tone, icon: tool.tone, content: tool.content, ...(tool.type === 'direcionar_setor' ? { sectorId: '' } : {}) } };
  ...
  return <main className={styles.editor}>
    ...
    <aside className={styles.library}><div className={styles.libraryTitle}><h2>Blocos</h2><button aria-label="Recolher biblioteca"><ChevronLeft size={17} /></button></div><p>Arraste para o canvas ou clique para adicionar</p>{catalog.isLoading && <p>Carregando catálogo...</p>}{catalog.error && <div className={styles.nodeError}>Não foi possível carregar os blocos aceitos pelo backend.</div>}<div className={styles.toolList}>{tools.map((tool) => { const Icon = tool.icon; return <button key={tool.type} draggable onDragStart={(event) => startDrag(event, tool)} onClick={() => addAtCenter(tool)} title={`Adicionar ${tool.label}`}><span className={styles[tool.tone]}><Icon size={18} /></span><div><strong>{tool.label}</strong><small>{tool.detail}</small></div><Plus size={15} /></button>; })}</div><div className={styles.tip}><MousePointer2 size={18} /><div><strong>Como montar</strong><p>Mensagem envia texto; Captura salva uma resposta; Condição escolhe um caminho; Direcionar setor encerra o bot e envia à fila humana.</p></div></div></aside>
