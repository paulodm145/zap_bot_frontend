...
export function FlowsList() {
  ...
  const columns = useMemo<DataTableColumn<FlowSummary>[]>(() => [
    ...
    { id: 'published', header: 'Publicação', width: '18%', hideOnMobile: true, render: (flow) => flow.publicado_at ? formatDate(flow.publicado_at) : '—' },
    { id: 'updated', header: 'Atualização', width: '18%', hideOnMobile: true, render: (flow) => formatDate(flow.updated_at) },
  ], []);
