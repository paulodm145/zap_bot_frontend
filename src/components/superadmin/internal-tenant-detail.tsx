...
export function InternalTenantDetailView({ tenantId }: { tenantId: string }) {
  ...
  return <InternalShell title={tenant.nome} subtitle={`Tenant ${tenant.public_id}`} actions={<Link href="/interno/tenants"><Button variant="ghost" icon={<ArrowLeft size={16} />}>Voltar</Button></Link>}>
    ...
    <section className={styles.actionsCard}><header><Activity size={18} /><div><h2>Ações administrativas</h2><p>Todas as alterações exigem motivo e geram auditoria.</p></div></header><div className={styles.actionButtons}>{tenant.status === 'ATIVO' && <Button icon={<LogIn size={16} />} disabled={impersonate.isPending} onClick={() => impersonate.mutate()}>{impersonate.isPending ? 'Conectando...' : 'Conectar como tenant'}</Button>}{tenant.status === 'ATIVO' && <Button variant="secondary" onClick={() => openStatus('SUSPENSO')}>Suspender tenant</Button>}{tenant.status === 'SUSPENSO' && <Button variant="secondary" onClick={() => openStatus('ATIVO')}>Reativar tenant</Button>}{!cancelled && <Button variant="secondary" onClick={() => setAction('plan')}>Alterar plano</Button>}{!cancelled && <Button variant="danger" onClick={() => openStatus('CANCELADO')}>Cancelar tenant</Button>}{(tenant.status === 'SUSPENSO' || cancelled) && <Button variant="danger" onClick={() => setDeleteOpen(true)}>Excluir definitivamente</Button>}{cancelled && <p>Tenant cancelado não pode ser reativado nem receber outro plano.</p>}</div>{impersonate.error && <div className={styles.error} role="alert">{isApiError(impersonate.error) ? impersonate.error.message : 'Não foi possível acessar este tenant.'}</div>}</section>
    ...
    {deleteOpen && <InternalTenantDeleteDialog tenantId={tenant.public_id} tenantName={tenant.nome} onClose={() => setDeleteOpen(false)} />}
  </InternalShell>;
}
