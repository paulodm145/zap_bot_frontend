...
import { Activity, ArrowLeft, Building2, CalendarDays, CreditCard, Database, ShieldAlert, UserRound, UsersRound } from 'lucide-react';
...
export function InternalTenantDetailView({ tenantId }: { tenantId: string }) {
  ...
  const mutationError = statusMutation.error ?? planMutation.error;
  async function submitAction(event: FormEvent) {
    ...
    } catch { /* apresentado no formulário */ }
  }
  function openStatus(status: 'ATIVO' | 'SUSPENSO' | 'CANCELADO') { setNextStatus(status); setAction('status'); setValidation(null); }
  return <InternalShell title={tenant.nome} subtitle={`Tenant ${tenant.public_id}`} actions={<Link href="/interno/tenants"><Button variant="ghost" icon={<ArrowLeft size={16} />}>Voltar</Button></Link>}><section className={styles.hero}><div className={styles.heroIcon}><Building2 size={25} /></div><div><span>IDENTIFICAÇÃO</span><h2>{tenant.nome}</h2><p>{tenant.public_id}</p></div><Badge tone={tenant.status === 'ATIVO' ? 'success' : tenant.status === 'PROVISIONANDO' ? 'warning' : 'neutral'}>{labelStatus(tenant.status)}</Badge></section>{tenant.mensagem_falha && <div className={styles.failure}><ShieldAlert size={18} /><div><strong>Falha de provisionamento</strong><p>{tenant.mensagem_falha}</p></div></div>}<section className={styles.facts}><article><Database size={18} /><span>Etapa</span><strong>{tenant.etapa_provisionamento ? labelStatus(tenant.etapa_provisionamento) : '—'}</strong></article><article><CreditCard size={18} /><span>Plano atual</span><strong>{tenant.plano?.nome ?? tenant.plano_nome ?? '—'}</strong></article><article><UsersRound size={18} /><span>Usuários</span><strong>{usuarios.length}</strong></article><article><CalendarDays size={18} /><span>Criado em</span><strong>{date(tenant.created_at)}</strong></article></section><div className={styles.columns}><section className={styles.card}><header><div><h2>Usuários vinculados</h2><p>Contas existentes no tenant.</p></div></header><div className={styles.list}>{usuarios.length ? usuarios.map((user) => <div key={user.public_id}><span><UserRound size={16} /></span><div><strong>{user.nome}</strong><small>{user.email}</small></div><Badge tone={user.ativo === false ? 'neutral' : 'success'}>{user.papel ?? (user.ativo === false ? 'Inativo' : 'Ativo')}</Badge></div>) : <p>Nenhum usuário retornado.</p>}</div></section><section className={styles.card}><header><div><h2>Histórico de assinaturas</h2><p>Planos manuais e recorrentes.</p></div></header><div className={styles.list}>{assinaturas.length ? assinaturas.map((subscription) => <div key={subscription.public_id}><span><CreditCard size={16} /></span><div><strong>{subscription.plano?.nome ?? 'Plano sem nome'}</strong><small>{date(subscription.created_at)}</small></div><Badge tone={subscription.status === 'ATIVA' ? 'success' : 'neutral'}>{subscription.status}</Badge></div>) : <p>Nenhuma assinatura retornada.</p>}</div></section></div><section className={styles.actionsCard}><header><Activity size={18} /><div><h2>Ações administrativas</h2><p>Todas as alterações exigem motivo e geram auditoria.</p></div></header><div className={styles.actionButtons}>{tenant.status === 'ATIVO' && <Button variant="secondary" onClick={() => openStatus('SUSPENSO')}>Suspender tenant</Button>}{tenant.status === 'SUSPENSO' && <Button variant="secondary" onClick={() => openStatus('ATIVO')}>Reativar tenant</Button>}{!cancelled && <Button variant="secondary" onClick={() => setAction('plan')}>Alterar plano</Button>}{!cancelled && <Button variant="danger" onClick={() => openStatus('CANCELADO')}>Cancelar tenant</Button>}{cancelled && <p>Tenant cancelado não pode ser reativado nem receber outro plano.</p>}</div></section>{action && <div className={styles.modalBackdrop} role="presentation"><form className={styles.modal} onSubmit={submitAction}><header><div><span>CONFIRMAÇÃO OBRIGATÓRIA</span><h2>{action === 'plan' ? 'Alterar plano' : `${labelStatus(nextStatus)} tenant`}</h2></div><button type="button" onClick={() => setAction(null)}>×</button></header>{action === 'plan' && <label><span>UUID do novo plano</span><input value={planId} onChange={(event) => setPlanId(event.target.value)} required /></label>}<label><span>Motivo operacional</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={4} required placeholder="Informe chamado, solicitação ou justificativa..." /></label><label className={styles.confirm}><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /> Confirmo que revisei o tenant e entendo o impacto.</label>{(validation || mutationError) && <div className={styles.error} role="alert">{validation ?? (isApiError(mutationError) ? mutationError.message : 'Não foi possível concluir a alteração.')}</div>}<footer><Button type="button" variant="ghost" onClick={() => setAction(null)}>Voltar</Button><Button type="submit" variant={nextStatus === 'CANCELADO' ? 'danger' : 'primary'} disabled={statusMutation.isPending || planMutation.isPending}>{statusMutation.isPending || planMutation.isPending ? 'Aplicando...' : 'Confirmar alteração'}</Button></footer></form></div>}</InternalShell>;
}
