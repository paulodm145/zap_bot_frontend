...
export function InternalTenantDeleteDialog({
  tenantId,
  tenantName,
  onClose,
}: {
  ...
  async function submit(event: FormEvent) {
    ...
    setValidation(null);
    try {
      await remove.mutateAsync({ password, tenantName: typedName, reason: reason.trim() });
    } catch (error) {
      if (isApiError(error) && error.status === 401) setPassword('');
    }
  }
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation">
      <form className={styles.modal} onSubmit={submit} aria-label="Excluir tenant definitivamente">
        <header>
          <div>
            <span>OPERAÇÃO IRREVERSÍVEL</span>
            <h2>Excluir definitivamente</h2>
          </div>
          <button type="button" disabled={remove.isPending} onClick={onClose}>
            ×
          </button>
        </header>
        <p className={styles.destructiveWarning}>
          O banco, usuários, sessões, conversas e mensagens serão removidos sem possibilidade de recuperação.
        </p>
        <label>
          <span>Senha atual do superadministrador</span>
          <input
            type="password"
            value={password}
        ...
        <label>
          <span>Digite o nome exato: {tenantName}</span>
          <input
            value={typedName}
        ...
        <label className={styles.confirm}>
          <input
            ...
            disabled={remove.isPending}
          />{' '}
          Confirmo a exclusão definitiva e irreversível deste tenant.
        </label>
        {(validation || error) && (
