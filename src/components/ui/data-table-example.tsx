...
const columns: DataTableColumn<Contact>[] = [
  { id: 'name', header: 'Contato', accessor: 'name', sortable: true, width: '26%', render: (row) => <div className={styles.contact}><span>{row.name.split(' ').map((part) => part[0]).slice(0, 2).join('')}</span><div><strong>{row.name}</strong><small>#{String(row.id).padStart(4, '0')}</small></div></div> },
  { id: 'phone', header: 'Telefone', accessor: 'phone', width: '19%', hideOnMobile: true },
  { id: 'sector', header: 'Setor', accessor: 'sector', width: '16%' },
  { id: 'status', header: 'Status', accessor: 'status', width: '16%', render: (row) => <Badge tone={row.status === 'Ativo' ? 'success' : row.status === 'Aguardando' ? 'warning' : 'neutral'}>{row.status}</Badge> },
