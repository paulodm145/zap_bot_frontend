    ...
    <section className={`${styles.root} ${styles[color]} ${className}`} aria-busy={loading}>
          ...
          <thead>
            <tr>
              {columns.map((column) => {
                ...
                const style = {
                  '--column-width': typeof column.width === 'number' ? `${column.width}px` : column.width,
                } as CSSProperties;
                return (
                  <th
                    key={column.id}
                    className={`${styles[column.align ?? 'left']} ${column.hideOnMobile ? styles.hideOnMobile : ''}`}
                    style={style}
              ...
              {rowActions && <th className={styles.actionsColumn}>{actionsHeader}</th>}
            </tr>
          </thead>
          <tbody>
            {loading
              ? Array.from({ length: Math.min(pagination.take, 6) }, (_, rowIndex) => (
              ...
              : data.map((row) => (
                  <tr key={getRowId(row)}>
                    ...
                    {rowActions && <td className={styles.rowActions}>{rowActions(row)}</td>}
                  </tr>
                ))}
          </tbody>
        </table>
      ...
      <footer className={styles.footer}>
        ...
        <div className={styles.pageSize}>
          <label htmlFor={pageSizeId}>Itens por página</label>
          <select
            id={pageSizeId}
            value={pagination.take}
            onChange={(event) => onPaginationChange({ skip: 0, take: Number(event.target.value) })}
          >
        ...
        <nav className={styles.pagination} aria-label="Paginação da tabela">
          <button type="button" onClick={() => changePage(1)} disabled={safePage === 1} aria-label="Primeira página">
          ...
          {pages.map((page, index) => (
            <span key={page} className={styles.pageSlot}>
              ...
              <button
                type="button"
                className={page === safePage ? styles.current : ''}
