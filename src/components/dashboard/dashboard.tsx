...
export function Dashboard() {
  return (
    <AppShell
      ...
      actions={
        <Link href="/fluxos/novo">
          <Button icon={<Plus size={17} />}>Novo fluxo</Button>
        </Link>
      }
    >
        ...
        {stats.map(({ label, value, change, up, icon: Icon }) => (
          <article className={styles.stat} key={label}>
            ...
            <div className={styles.statValue}>
              <strong>{value}</strong>
              <em className={up ? styles.positive : styles.good}>
        ...
        <section className={styles.card}>
          <header>
            <div>
              ...
              <p>Mensagens recebidas nos últimos 7 dias</p>
            </div>
          ...
          <div className={styles.chart} aria-label="Gráfico ilustrativo">
            ...
            <div className={styles.bars}>
              {[42, 62, 48, 78, 67, 91, 72].map((height, index) => (
                <div key={index}>
        ...
        <aside className={styles.card}>
          ...
          <div className={styles.queue}>
            <div>
              ...
              <p>
                <strong>8</strong> aguardando na fila
              </p>
            </div>
            <div>
              <span className={styles.blueDot} />
              <p>
                <strong>16</strong> em atendimento
              </p>
            </div>
            <div>
              <span className={styles.grayDot} />
              <p>
                <strong>7</strong> atendentes online
              </p>
            </div>
          </div>
          <Button variant="secondary">Abrir painel de atendimento</Button>
        </aside>
      </div>
      <section className={`${styles.card} ${styles.flows}`}>
        <header>
          <div>
            <h2>Fluxos recentes</h2>
            <p>Acompanhe o desempenho das suas automações</p>
          </div>
          <Link href="/fluxos">
        ...
        <div className={styles.flowList}>
          {flows.map((flow) => (
            <div className={styles.flowRow} key={flow.name}>
              ...
              <Badge tone={flow.status === 'Publicado' ? 'success' : 'neutral'}>{flow.status}</Badge>
              <span className={styles.conversations}>
                <strong>{flow.conversations}</strong>
                <small>conversas</small>
              </span>
              <button aria-label={`Ações de ${flow.name}`}>
