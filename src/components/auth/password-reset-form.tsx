...
export function PasswordResetForm() {
  ...
  async function submit(event: FormEvent) {
    ...
    if (!passwordPattern.test(password))
      return setValidation('Use de 12 a 128 caracteres, incluindo maiúscula, minúscula e número.');
  ...
  const apiMessage = reset.error
    ? isApiError(reset.error) && reset.error.code === 'TOKEN_RECUPERACAO_INVALIDO'
      ? 'Este link é inválido, expirou ou já foi utilizado. Solicite um novo.'
    ...
    return (
      <div className={`${styles.formCard} page-enter`}>
        ...
        <header>
          ...
          <p>Sua nova senha já está ativa. As sessões anteriores foram encerradas por segurança.</p>
        </header>
        <Link href="/login">
          <Button>
            Ir para o login
  ...
  return (
    <div className={`${styles.formCard} page-enter`}>
      <header>
        <span className={styles.step}>NOVA SENHA</span>
        <h2>Crie uma senha segura</h2>
        <p>O link será usado uma única vez e todas as sessões anteriores serão encerradas.</p>
      </header>
