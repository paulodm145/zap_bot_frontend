# Tarefas — módulos tenant

- [x] **TEN-01 — Perfil e navegação por papel** — `feat/tenant-profile-navigation` — `feat(tenant): add profile and operational navigation`
- [x] **TEN-02 — Cadastro de setores** — `feat/tenant-sectors` — `feat(tenant): integrate sector management`
- [x] **TEN-03 — Cadastro de usuários** — `feat/tenant-users` — `feat(tenant): integrate user management`
- [x] **TEN-04 — Atendimento e chat** — `feat/tenant-chat` — `feat(tenant): integrate conversation workspace`
- [x] **TEN-05 — Dados da empresa** — `feat/tenant-company` — `feat(tenant): integrate company profile`
- [x] **TEN-06 — Contas WhatsApp** — `feat/tenant-whatsapp-accounts` — `feat(tenant): integrate whatsapp accounts`
- [x] **TEN-07 — Validação final** — `test/tenant-modules` — `test(tenant): validate operational modules`
- [x] **TEN-08 — Padronizar grids, modais e dados empresariais** — `fix/tenant-crud-design` — `fix(tenant): standardize crud experience`
- [x] **TEN-09 — Corrigir overlay dos modais** — `fix/tenant-modal-overlay` — `fix(tenant): render crud modals in viewport portal`
- [x] **TEN-10 — Corrigir validação da listagem de setores** — `fix/sector-validation-contract` — `fix(sectors): omit empty search parameter`
- [x] **TEN-11 — Adicionar edição e toggle de usuários** — `feat/tenant-user-edit-toggle` — `feat(users): add edit flow and status toggle`
- [x] **TEN-12 — Melhorar legibilidade e feedback** — `fix/tenant-readability-feedback` — `fix(ui): improve tables forms and error feedback`
- [x] **TEN-13 — Corrigir tipografia do formulário empresarial** — `fix/company-form-typography` — `fix(company): increase form typography`
- [x] **TEN-14 — Migrar Contas WhatsApp para Evolution API** — `feat/contas-whatsapp-evolution-api` — `feat(tenant): adapt whatsapp accounts to evolution api`. Backend trocou de WhatsApp Cloud API (Meta) para Evolution API (self-hosted); onboarding por token/phoneNumberId/wabaId virou pareamento por QR code. Removida a seleção de "fluxo de entrada" no formulário de criação — o campo já não tinha suporte real no backend (`POST /contas-whatsapp` sempre rejeitou esse campo por schema estrito), então nunca funcionou; ver nota em TAREFAS-NOVOS-ENDPOINTS-FRONTEND.md se essa automação for priorizada.
